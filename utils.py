...
def save_checkpoint(state, is_best, epoch, checkpoint, dataset, ins_name=-1):
    ...
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'best_'+dataset+'.pth.tar'))
        logger.info('Best checkpoint copied to best_'+dataset+'.pth.tar')
