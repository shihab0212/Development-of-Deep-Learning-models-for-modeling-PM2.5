...
class DateDataset(Dataset):
    def __init__(self, data_path, data_name, num_class):
        self.data = np.load(os.path.join(data_path, f'test_dates{data_name}.npy'))
...
class TestDataset(Dataset):
    def __init__(self, data_path, data_name, num_class):
        ...
        self.date = np.load(os.path.join(data_path, f'test_dates_{data_name}.npy'))
    ...
    def __getitem__(self, index):
        return (self.data[index,:,:-1],int(self.data[index,0,-1]),self.v[index], self.label[index], self.date[index])

class Test_Final_Dataset(Dataset):
    def __init__(self, data_path, data_name, num_class):
        self.data = np.load(os.path.join(data_path, f'test_data_{data_name}.npy'))
        self.v = np.load(os.path.join(data_path, f'test_v_{data_name}.npy'))
        self.label = np.load(os.path.join(data_path, f'test_label_{data_name}.npy'))
